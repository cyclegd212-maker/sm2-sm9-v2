...
{
    size_t i;
    for (i = 0; i < len; i++) message[i] = (uint8_t)((i * 173u + len * 19u + 7u) & 0xffu);
}

static int warmup_roundtrips(const GONG2025_PUBLIC_PARAMS *pp,
    const GONG2025_PKI_KEY *sender, const GONG2025_CLC_KEY *receiver,
    const uint8_t *message, size_t message_len, unsigned count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (gong2025_signcrypt(pp, sender, id_s, sizeof(id_s)-1,
                receiver, id_r, sizeof(id_r)-1, message, message_len, &ct) != GONG2025_OK
            || gong2025_unsigncrypt(pp, sender, id_s, sizeof(id_s)-1,
...
int main(int argc, char **argv)
{
    ...
    if (gong2025_setup(&msk, &pp) != GONG2025_OK
        || gong2025_pki_keygen(&sender) != GONG2025_OK
        || gong2025_clc_keygen(&msk, &pp, id_r, sizeof(id_r)-1, &receiver) != GONG2025_OK) {
        fprintf(stderr, "Gong 2025 setup/key generation failure\n");
        goto end;
    }
    if (!warmup_roundtrips(&pp, &sender, &receiver,
            message, opt.message_bytes, opt.warmup)) {
        fprintf(stderr, "Gong 2025 warmup/correctness failure\n");
        goto end;
    }
    fp = open_raw(&opt);
    ...
    for (i = 0; i < opt.iterations; i++) {
        ...
        t0 = now_ns();
        if (gong2025_signcrypt(&pp, &sender, id_s, sizeof(id_s)-1,
                &receiver, id_r, sizeof(id_r)-1,
                message, opt.message_bytes, &ct) != GONG2025_OK) {
            fprintf(stderr, "Gong signcrypt failure at iteration %u\n", i);
            gong2025_ciphertext_cleanup(&ct); goto end;
        }
        t1 = now_ns();
        if (t1 < t0 || !row(fp, &opt, "gong2025_sender_signcrypt", i, t1-t0)) {
            gong2025_ciphertext_cleanup(&ct); goto end;
        }

        t0 = now_ns();
        if (gong2025_unsigncrypt(&pp, &sender, id_s, sizeof(id_s)-1,
                &receiver, id_r, sizeof(id_r)-1, &ct, out, &outlen) != GONG2025_OK) {
            fprintf(stderr, "Gong unsigncrypt failure at iteration %u\n", i);
            gong2025_ciphertext_cleanup(&ct); goto end;
        }
        t1 = now_ns();
        if (t1 < t0 || !row(fp, &opt, "gong2025_unsigncrypt", i, t1-t0)) {
            gong2025_ciphertext_cleanup(&ct); goto end;
        }
        if (outlen != opt.message_bytes
            || (opt.message_bytes && memcmp(message, out, opt.message_bytes) != 0)) {
            fprintf(stderr, "Gong correctness failure at iteration %u\n", i);
            gong2025_ciphertext_cleanup(&ct); goto end;
        }
        gong2025_ciphertext_cleanup(&ct);
    }
