...
{
    size_t i;
    for (i = 0; i < len; i++) buf[i] = (uint8_t)(i * 29u + len * 7u + 3u);
}

static void u64be(uint64_t v, uint8_t out[8])
{
    int i;
    for (i = 7; i >= 0; i--) { out[i] = (uint8_t)v; v >>= 8; }
}
    ...
    const SM2_Z256_POINT *R_r, const SM2_Z256_POINT *P_r, sm2_z256_t out)
{
    ...
    u64be(65, n); memcpy(p, n, 8); p += 8; memcpy(p, octets, 65); p += 65;
    return digest_to_nonzero_scalar(H1_DOMAIN, sizeof(H1_DOMAIN)-1,
        body, (size_t)(p-body), out);
}
    ...
    const GONG2025_CIPHERTEXT *ct)
{
    ...
    sm2_z256_t h, hx, a, z, h_inv, xr_plus_dr, h1_again;

    if (!test_h1(id_r, id_r_len, &receiver->R_r, &receiver->P_r, h1_again)
        || sm2_z256_cmp(h1_again, receiver->h_r) != 0) return 0;
    ...
    if (!test_h4(id_s, id_s_len, id_r, id_r_len, sender, receiver, pp, ct, h)) return 0;
...
static int rejects(const GONG2025_PUBLIC_PARAMS *pp, const GONG2025_PKI_KEY *sender,
    const GONG2025_CLC_KEY *receiver, const GONG2025_CIPHERTEXT *ct,
    uint8_t *out, size_t cap)
{
    ...
    size_t n = cap;
    memset(out, 0, cap);
    return gong2025_unsigncrypt(pp, sender, id_s, sizeof(id_s)-1,
        receiver, id_r, sizeof(id_r)-1, ct, out, &n) == GONG2025_ERR;
}
...
static int tamper_rejection(void)
{
    ...
    GONG2025_MASTER_KEY msk;
    GONG2025_PUBLIC_PARAMS pp;
    GONG2025_PKI_KEY sender, altered_sender;
    GONG2025_CLC_KEY receiver, altered_receiver;
    GONG2025_CIPHERTEXT ct;
    uint8_t message[N], out[N];
    uint8_t saved_c0;
    sm2_z256_t one, saved_S;
    SM2_Z256_POINT G, saved_T1;
    size_t outlen;
    int ret;
    int ok = 0;

    memset(&msk, 0, sizeof(msk)); memset(&pp, 0, sizeof(pp));
    memset(&sender, 0, sizeof(sender)); memset(&receiver, 0, sizeof(receiver));
    gong2025_ciphertext_init(&ct);
    ...
    CHECK(gong2025_clc_keygen(&msk, &pp, id_r, sizeof(id_r)-1, &receiver) == GONG2025_OK);
    CHECK(gong2025_signcrypt(&pp, &sender, id_s, sizeof(id_s)-1,
        &receiver, id_r, sizeof(id_r)-1, message, N, &ct) == GONG2025_OK);

    saved_c0 = ct.c[0]; ct.c[0] ^= 1;
    CHECK(rejects(&pp, &sender, &receiver, &ct, out, N)); ct.c[0] = saved_c0;

    sm2_z256_copy(saved_S, ct.S); sm2_z256_modn_add(ct.S, ct.S, one);
    CHECK(rejects(&pp, &sender, &receiver, &ct, out, N)); sm2_z256_copy(ct.S, saved_S);

    memcpy(&saved_T1, &ct.T1, sizeof(saved_T1)); sm2_z256_point_add(&ct.T1, &ct.T1, &G);
    CHECK(rejects(&pp, &sender, &receiver, &ct, out, N)); memcpy(&ct.T1, &saved_T1, sizeof(ct.T1));

    memcpy(&altered_sender, &sender, sizeof(sender));
    sm2_z256_point_add(&altered_sender.P_s, &altered_sender.P_s, &G);
    CHECK(rejects(&pp, &altered_sender, &receiver, &ct, out, N));

    memcpy(&altered_receiver, &receiver, sizeof(receiver));
    sm2_z256_point_add(&altered_receiver.P_r, &altered_receiver.P_r, &G);
    CHECK(rejects(&pp, &sender, &altered_receiver, &ct, out, N));

    memcpy(&altered_receiver, &receiver, sizeof(receiver));
    sm2_z256_point_add(&altered_receiver.R_r, &altered_receiver.R_r, &G);
    CHECK(rejects(&pp, &sender, &altered_receiver, &ct, out, N));

    memcpy(&altered_receiver, &receiver, sizeof(receiver));
    sm2_z256_point_add(&altered_receiver.X_r, &altered_receiver.X_r, &G);
    CHECK(rejects(&pp, &sender, &altered_receiver, &ct, out, N));

    memcpy(&altered_receiver, &receiver, sizeof(receiver));
    sm2_z256_modn_add(altered_receiver.x_r, altered_receiver.x_r, one);
    if (sm2_z256_is_zero(altered_receiver.x_r)) sm2_z256_modn_add(altered_receiver.x_r, altered_receiver.x_r, one);
    outlen = N; memset(out, 0, N);
    ret = gong2025_unsigncrypt(&pp, &sender, id_s, sizeof(id_s)-1,
        &altered_receiver, id_r, sizeof(id_r)-1, &ct, out, &outlen);
    CHECK(ret == GONG2025_ERR || outlen != N || memcmp(out, message, N) != 0);
...
int main(void)
{
    ...
    size_t i;
    for (i = 0; i < sizeof(sizes)/sizeof(sizes[0]); i++) if (!roundtrip(sizes[i])) return 1;
    if (!tamper_rejection()) return 1;
