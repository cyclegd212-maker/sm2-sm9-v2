#!/usr/bin/env python3
"""Compare archived V2 raw samples with Gong et al. 2025 PKI->CLC raw samples."""
...
def compare(v2_rows:list[dict[str,str]],gong_rows:list[dict[str,str]],*,expected_sizes:tuple[int,...]=DEFAULT_SIZES,expected_n:int=1000)->tuple[dict[str,str],list[dict[str,object]]]:
    v2_run,v2_commit,v2_gm=identity(v2_rows,"V2"); gong_run,gong_commit,gong_gm=identity(gong_rows,"Gong")
    if v2_gm!=gong_gm: raise ValueError(f"GmSSL commit mismatch: V2={v2_gm}, Gong={gong_gm}; same-library comparison required")
    v2=phase_means(v2_rows,label="V2",allowed_phases=V2_PHASES,expected_sizes=expected_sizes,expected_n=expected_n)
    gong=phase_means(gong_rows,label="Gong",allowed_phases=GONG_PHASES,expected_sizes=expected_sizes,expected_n=expected_n)
    rows=[]
    for size in expected_sizes:
        vo=v2[(size,"online_signcrypt")]/1_000_000.0; vt=v2[(size,"sender_total")]/1_000_000.0; vu=v2[(size,"unsigncrypt")]/1_000_000.0
        gs=gong[(size,"gong2025_sender_signcrypt")]/1_000_000.0; gu=gong[(size,"gong2025_unsigncrypt")]/1_000_000.0
        if gs<=0 or gu<=0: raise ValueError("Gong mean latency must be positive")
        rows.append({"message_bytes":size,"v2_online_ms":vo,"v2_sender_total_ms":vt,"v2_unsigncrypt_ms":vu,"gong_signcrypt_ms":gs,"gong_unsigncrypt_ms":gu,"online_reduction_pct":(gs-vo)/gs*100.0,"sender_total_delta_pct":(vt-gs)/gs*100.0,"unsigncrypt_delta_pct":(vu-gu)/gu*100.0})
    meta={"v2_run_id":v2_run,"v2_commit":v2_commit,"gong_run_id":gong_run,"gong_commit":gong_commit,"gmssl_commit":v2_gm}
    return meta,rows

def write_comparison(path:Path,meta:dict[str,str],rows:list[dict[str,object]])->None:
    path.parent.mkdir(parents=True,exist_ok=True)
    fields=("v2_run_id","v2_commit","gong_run_id","gong_commit","gmssl_commit","message_bytes","v2_online_ms","v2_sender_total_ms","v2_unsigncrypt_ms","gong_signcrypt_ms","gong_unsigncrypt_ms","online_reduction_pct","sender_total_delta_pct","unsigncrypt_delta_pct")
...
def self_test()->None:
    ...
    gong=synthetic_rows(run_id="gong-self",commit="gong-c",gmssl_commit="same",phase_values={"gong2025_sender_signcrypt":[1000000,1000000],"gong2025_unsigncrypt":[1500000,1500000]})
    meta,rows=compare(v2,gong,expected_sizes=(20,),expected_n=2); r=rows[0]
    assert meta["gmssl_commit"]=="same" and math.isclose(float(r["online_reduction_pct"]),50.0)
    assert math.isclose(float(r["sender_total_delta_pct"]),100.0) and math.isclose(float(r["unsigncrypt_delta_pct"]),100.0)
    with tempfile.TemporaryDirectory() as td:
        out=Path(td)/"comparison.csv"; write_comparison(out,meta,rows); txt=out.read_text(encoding="utf-8"); assert "online_reduction_pct" in txt and "gong-self" in txt
    bad=synthetic_rows(run_id="gong-bad",commit="gong-c",gmssl_commit="different",phase_values={"gong2025_sender_signcrypt":[1000000,1000000],"gong2025_unsigncrypt":[1500000,1500000]})
    ...
    print("compare_v2_gong2025 self-test: ok")
...
def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("--v2-raw",type=Path); p.add_argument("--gong-raw",type=Path); p.add_argument("--out",type=Path); p.add_argument("--expected-sizes",type=parse_sizes,default=DEFAULT_SIZES); p.add_argument("--expected-n",type=int,default=1000); p.add_argument("--self-test",action="store_true"); a=p.parse_args()
    if a.self_test: self_test(); return 0
    if a.v2_raw is None or a.gong_raw is None or a.out is None: p.error("--v2-raw, --gong-raw, and --out are required unless --self-test")
    if a.expected_n<=0: p.error("--expected-n must be positive")
    meta,rows=compare(read_raw(a.v2_raw),read_raw(a.gong_raw),expected_sizes=a.expected_sizes,expected_n=a.expected_n); write_comparison(a.out,meta,rows); print(f"wrote {len(rows)} comparison rows to {a.out}"); return 0
