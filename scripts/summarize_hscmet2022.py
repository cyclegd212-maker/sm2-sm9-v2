#!/usr/bin/env python3
"""Summarize raw HSC-MET 2022 PKI->CLC benchmark samples."""
...
def summarize(rows:list[dict[str,str]],*,expected_sizes:tuple[int,...]|None=None,expected_n:int|None=None)->list[dict[str,object]]:
    ...
    for row in rows:
        ...
        phase=row["phase"]
        if phase not in HSCMET_PHASES: raise ValueError(f"unexpected HSC-MET phase: {phase!r}")
        groups[(size,phase)].append((it,ns))
    if expected_sizes is not None:
        expected={(s,p) for s in expected_sizes for p in HSCMET_PHASES}
        if set(groups)!=expected:
            raise ValueError(f"HSC-MET groups mismatch; missing={sorted(expected.difference(groups))}, extra={sorted(set(groups).difference(expected))}")
    out=[]
...
def self_test()->None:
    rows=[]
    values={(20,"hscmet2022_sender_signcrypt"):[10,20,30,40,100],(20,"hscmet2022_unsigncrypt"):[50,60,70,80,90]}
    for (size,phase),samples in values.items():
        for i,ns in enumerate(samples): rows.append({"run_id":"self","commit":"hscmet-self","gmssl_commit":"gmssl-self","message_bytes":str(size),"phase":phase,"iteration":str(i),"ns":str(ns)})
    result=summarize(rows,expected_sizes=(20,),expected_n=5); by={str(r["phase"]):r for r in result}; a=by["hscmet2022_sender_signcrypt"]
    ...
    print("summarize_hscmet2022 self-test: ok")
