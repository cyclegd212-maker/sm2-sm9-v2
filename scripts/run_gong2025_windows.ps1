...
if ($LASTEXITCODE -ne 0 -or -not $GitHead) {
    throw "formal Gong benchmark must run from a Git checkout"
}
...
$RunId = "gong2025-pki-clc-ryzen8845h-$Timestamp"
$ArtifactDir = Join-Path $Root "artifacts\$RunId"
$BuildDir = Join-Path $Root "native\build-gong2025-pki-clc"
...
$BenchExe = Get-ChildItem -Path $BuildDir -Recurse -File -Filter "bench_gong2025_pki_clc.exe" | Select-Object -First 1
if (-not $BenchExe) { throw "bench_gong2025_pki_clc.exe not found below $BuildDir" }
...
$EnvRecord = [ordered]@{
    ...
    expected_phases = @("gong2025_sender_signcrypt", "gong2025_unsigncrypt")
    platform = "Windows"
...
Write-Host "Gong 2025 formal benchmark complete"
Write-Host "Run ID: $RunId"
Write-Host "Artifact directory: $ArtifactDir"
Write-Host "Raw samples: $Raw"
Write-Host "Summary: $Summary"
if ($V2Raw) { Write-Host "V2/Gong comparison: $Comparison" }
