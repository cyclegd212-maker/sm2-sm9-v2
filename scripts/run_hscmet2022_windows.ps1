param(
    ...
    [string]$CCompiler = "C:\msys64\ucrt64\bin\gcc.exe"
)
...
if ($LASTEXITCODE -ne 0 -or -not $GitHead) {
    throw "formal HSC-MET benchmark must run from a Git checkout"
}
$GitStatus = (git status --porcelain=v1 2>$null) -join "`n"
if ($LASTEXITCODE -ne 0) { throw "unable to obtain git status" }
if ($GitStatus) { throw "working tree is not clean; commit/stash changes before formal benchmark" }

$Timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$RunId = "hscmet2022-pki-clc-n2-k2-ryzen8845h-$Timestamp"
$ArtifactDir = Join-Path $Root "artifacts\$RunId"
$BuildDir = Join-Path $Root "native\build-hscmet2022-pki-clc"
...
if (Test-Path $Raw) { throw "refusing to overwrite existing raw CSV: $Raw" }
...
$ConfigureArgs = @(
    ...
    "-DCMAKE_C_COMPILER=$ResolvedCompiler"
)
Invoke-NativeLogged -Log $BuildLog -Command "cmake" -Arguments $ConfigureArgs
Invoke-NativeLogged -Log $BuildLog -Command "cmake" -Arguments @("--build", $BuildDir, "--config", "Release")
Invoke-NativeLogged -Log $TestLog -Command "ctest" -Arguments @("--test-dir", $BuildDir, "-C", "Release", "--output-on-failure")

$BenchExe = Get-ChildItem -Path $BuildDir -Recurse -File -Filter "bench_hscmet2022.exe" | Select-Object -First 1
if (-not $BenchExe) { throw "bench_hscmet2022.exe not found below $BuildDir" }
...
$SummaryArgs = @(
    "scripts/summarize_hscmet2022.py",
    "--raw", $Raw,
    "--out", $Summary,
...
$EnvRecord = [ordered]@{
    ...
    hscmet_commit = $GitHead
    gmssl_commit = $GmsslCommit
    gmssl_root = $ResolvedGmsslRoot
    build_type = "Release"
    generator = "Ninja"
    c_compiler = $ResolvedCompiler
    hscmet_n = 2
    hscmet_k = 2
    h2_adaptation = "SM3-KDF point mask of message_bytes+32 bytes"
    h3_bytes = 64
    warmup = $Warmup
    iterations = $Iterations
    message_bytes = @(20, 128, 1024, 4096)
    expected_phases = @("hscmet2022_sender_signcrypt", "hscmet2022_unsigncrypt")
    platform = "Windows"
    ...
    compiler_cc = Command-Text $ResolvedCompiler @("--version")
...
if ($V2Raw) {
    ...
    $CompareArgs = @(
        "scripts/compare_v2_hscmet2022.py",
...
Write-Host "HSC-MET 2022 formal benchmark complete"
Write-Host "Run ID: $RunId"
Write-Host "Artifact directory: $ArtifactDir"
Write-Host "Raw samples: $Raw"
Write-Host "Summary: $Summary"
if ($V2Raw) { Write-Host "V2/HSC-MET comparison: $Comparison" }
