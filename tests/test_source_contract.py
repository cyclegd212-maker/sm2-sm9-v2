...
def test_native_benchmark_covers_qb_and_g1_add_microbenchmarks():
    bench = (ROOT / "native" / "bench" / "bench_v2.c").read_text(encoding="utf-8")
    validator = (ROOT / "native" / "bench" / "validate_evidence.py").read_text(encoding="utf-8")
    for phase in ["sm9_qb_compute", "sm9_g1_add"]:
        assert f'"{phase}"' in bench
        assert f'"{phase}"' in validator
